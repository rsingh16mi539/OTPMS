...
def get_student_details(request):
    if not request.session.get("name"):
        ...
        if Admin.objects.filter(email=get_mail).exists():
            ...
            if not Student.objects.filter(college_id=roll,y_id=year).exists():
                return HttpResponse("Student not found")
            obj = Student.objects.get(college_id=roll,y_id=year)
            context = {}
            context["student"]=obj
...
def web_apply_company(request):
    ...
    if applied == "false":
        ...
        if reg_end and timezone.now()>reg_end:
            ...
            elif(float(student.average)<=float(company.be_criteria)):
